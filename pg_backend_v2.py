...
from pg_ui_v4 import Ui_PasswordGenerator
...
class PasswordGeneratorApp(QMainWindow, Ui_PasswordGenerator):
    ...
    def generate_passwords(self):
        ...
        if not (5 < lnth <= 25 and 1 < num <= 25):
            self.passwords_output.setPlainText(
                "Error has occurred. Please keep in mind that the amount can be set between 1 and 25 while the length between 6 and 25.")
            return
        ...
        if not characters:
            self.passwords_output.setPlainText(
                "Please select at least one option (Numbers, Symbols, Lowercase letters or Uppercase letters).")
            return
    ...
    def calculate_security_rating(self, lnth, include_numbers, include_symbols, include_lowercase, include_uppercase):
        rating = 0

        length_bonus = min((lnth - 6) * 1.33, 40)
        rating += length_bonus
